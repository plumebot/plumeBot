...
| B-049 | ✅ 已完成 | 描述缓存键的 URL 规范化兜底 | 描述缓存修复（B-027 增强） | 已完成 | **已完成**：不再做 URL 归一（易误判），改为**内容寻址兜底**——`EinoMediaDescriber.Describe` 遇「http(s) URL 且无 FileHash」时先本地拉一次字节算内容 md5 作为缓存键（`key_origin=fetched_md5`），字节已在手则描述直接走 base64（不依赖 provider 侧拉图，顺带规避 qpic 不可达）；同图跨轮/跨 URL 命中同一内容键，缓存失效根因消除（QQ 图片 URL 带时效签名，原先 `url:<原串>` 键每轮不同）。代价：仅该场景多付一次本地拉取（带 FileHash 的主路径仍 URL 直传不下载）；失败冷却判定保持**先于预拉取**，不可达图不逐轮白付拉取。单测 `TestEinoMediaDescriberHTTPNoFileHashFetch`（同图异 URL 只调模型 1 次 + 键为内容 md5）与 `...HTTPNoFileHashCooldown`（冷却期内不再拉图）锁定。另补全链路调试日志：convert 记 FileHash 有无、describer 记 key/key_origin/缓存命中·未命中/写入缓存、memory 组装记描述成功 |
