...
| B-048 | 后置 | 会话窗口删除语义待定案 | P7-003（对话历史） | 用户拍板后 | 「对话历史」仅只读。后续「删除」需先定案：① 仅删内存窗口条目——`RemoveByIDs` 现语义为压缩批次归档，需另加「按前端传入 message_id 从窗口移除」的入口（窗口为运行态，重启即失）；或 ② 连 SQLite `messages` 一起删——影响 Agent 记忆与摘要归档，属数据破坏。另「更早历史分页」（SQLite 全量，需先处理 B-033 私聊维度）列为后续方向。**P7-003 补充**：同一栏目已展示摘要热链（「更早的对话纪要」），删除语义若涉及窗口，则还需一并定「摘要是否可删」（`SummaryStore` 热链 + conversation_summary 归档） |
| B-049 | ✅ 已完成 | 描述缓存键的 URL 规范化兜底 | 描述缓存修复（B-027 增强） | 已完成 | **已完成**，分两层：① **根因（实机确认）**——`normalizeFileHash` 只剥 `.image/.pic` 固定后缀，而 NapCat 实机给的是 `<大写md5>.jpg`（如 `A5FF8510A27F2DD0C604078B1223D6CA.jpg`）→ 长度 36 被整体拒收 → `FileHash` 为空 → 缓存键退化为带时效签名的 `url:<原串>`、同图每轮不命中。修复：改为**剥首个点之后的全部**（命名形态不可穷举），大小写归一保持。② **兜底**——`EinoMediaDescriber.Describe` 遇「http(s) URL 且无 FileHash」时先本地拉一次字节算 md5 作键（`key_origin=fetched_md5`），字节已在手则描述直接走 base64（不依赖 provider 侧拉图，顺带规避 qpic 不可达）；失败冷却判定保持**先于预拉取**，不可达图不逐轮白付拉取。测：`TestNormalizeFileHash/实机大写jpg后缀`、`TestToParts/图片file为实机形态`、`TestEinoMediaDescriberHTTPNoFileHashFetch`、`...HTTPNoFileHashCooldown`。诊断日志：convert 首次缺 FileHash 记 Warn（带 url/file/file_md5 原值）、describer 记 key/key_origin/缓存命中·未命中/写入缓存 |
